...
if (isset($_SESSION['username']) &&
   ...
   $dateJoined = date("Y-m-d");  // automatically sets today's date
   $status = "Active"; 

   $data = "fname=".$first_name."&uname=".$username."&email=".$email."&bd=".$date_of_birth."&lname=".$last_name . "&status=" . urlencode($status);
    ...
    if (!Validation::name($first_name)) {
       ...
       if($user->is_username_unique($username)){
           ...
           $user_data = [$username, $first_name, $last_name, $email, $date_of_birth, $password , $dateJoined, $status ];
